...
class MockCVPDatabase:
    ...
    def __init__(self, devices: dict = None, containers: dict = None, configlets: dict = None):
        ...
        self.containers = containers if containers is not None else CVP_DATA_CONTAINERS_INIT.copy()
    ...
    def _get_container_by_key(self, key: str) -> dict:
        for container in self.containers.values():
            if container.get(FIELD_KEY) == key:
                return container
    ...
    def _count_container_child(self, id: str) -> int:
        count = 0
        for container in self.containers.values():
            if container[FIELD_PARENT_ID] == id:
                count += 1
        return count
    ...
    def add_container(self, container_name, parent_name, parent_key):
        ...
        self.containers[container_name] = {FIELD_NAME: container_name,
                                           FIELD_KEY: CONTAINER_KEY,
                                           FIELD_PARENT_ID: parent_key}
    ...
    def apply_configlets_to_container(self, app_name, container,
        ...
        if container[FIELD_NAME] not in self.containers:
            raise CvpNotFoundError(f'Container {container[FIELD_NAME]} not found')
        for configlet in new_configlets:
            if configlet[FIELD_NAME] not in self.configlets:
                raise CvpNotFoundError(f'Configlet {configlet[FIELD_NAME]} not found')
            self.configlets[configlet[FIELD_NAME]][FIELD_CONTAINER_ATTACHED].append(container[FIELD_NAME])
        return self._get_response(True)
