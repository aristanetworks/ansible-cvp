...
logger = setup_custom_logger('cv_configlet_v3')
...
def CvContainerTools_Manager(request):
    ...
    request.cls.provisioner = CloudvisionProvisioner(server=server, token=user_token)
    ...
    logger.info("CV Proviosioning done at {}.".format(time_log()))
    yield
...
class Test_CvConfiglet_Unit():
    ...
    def test_cv_connection(self, test_configlet, check_mode):
        requests.packages.urllib3.disable_warnings()
        logger.debug("Class is connected to CV at {}".format(time_log()))
        assert True

    @pytest.mark.dependency(depends=["authentication"], scope='class')
    def test_configlet_data_from_cv(self, test_configlet, check_mode):
        logger.debug('Start to get configlet data from CV {}'.format(time_log()))
        cv_data = self.cv_configlet.get_configlet_data_cv(configlet_name=test_configlet['name'])
        logger.debug('Got all configlet data from CV {}'.format(time_log()))
        self.cv_configlet._ansible.check_mode = check_mode
        logger.debug('CV data for configlet {0} is {1}'.format(test_configlet['name'], cv_data))
    ...
    def test_str_compare(self, test_configlet, check_mode):
        ...
        if diff_data[0]:
            logger.info('Diff is : {}'.format(diff_data[1]))
        else:
    ...
    def test_create_configlet(self, test_configlet, check_mode):
        ...
        logger.debug('API result is: {}'.format(api_call[0].results))
    ...
    def test_update_configlet(self, test_configlet, check_mode):
        ...
        logger.debug('Update - API result is: {}'.format(api_call[0].results))
        assert type(api_call[0] is CvApiResult)
        assert api_call[0].results['success'] is True
        ...
        if check_mode is False:
            ...
            api_call = self.cv_configlet.update(to_update=[configlet], note='Updated by pytest - {}'.format(now.strftime("%m/%d/%Y, %H:%M:%S")))
            logger.debug('Revert to expected - API result is: {}'.format(api_call[0].results))
...
class Test_CvConfiglet_Manager():
    ...
    @pytest.mark.dependency(name='authentication')
    @pytest.mark.parametrize("configlet_inventory", generate_CvConfigletTools_content(configlet_inputs=SYSTEM_CONFIGLETS_TESTS, is_present_state=True), ids=['configlet-already-declared'])
    ...
    @pytest.mark.dependency(depends=["authentication"], scope='class')
    @pytest.mark.parametrize("configlet_inventory", generate_CvConfigletTools_content(configlet_inputs=SYSTEM_CONFIGLETS_TESTS, is_present_state=True), ids=['configlet-already-declared'])
    def test_apply_present(self, configlet_inventory):
        ...
        logger.debug('API call response is: {}'.format(api_result.content))
    ...
    @pytest.mark.dependency(depends=["authentication"], scope='class')
    @pytest.mark.parametrize("configlet_inventory", generate_CvConfigletTools_content(configlet_inputs=SYSTEM_CONFIGLETS_TESTS, is_present_state=False), ids=['configlet-not-already-declared'])
