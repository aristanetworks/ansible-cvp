...
class TestCvDeviceTools():
    ...
    def test_search_by_getter_setter(self, CV_DEVICE):
        self.inventory.search_by = ApiFields.device.SYSMAC
        assert self.inventory.search_by == ApiFields.device.SYSMAC
        logging.info(
            "Setter & Getter for search_by using {} is valid".format(ApiFields.device.SYSMAC))
        self.inventory.search_by = ApiFields.device.FQDN
        assert self.inventory.search_by == ApiFields.device.FQDN
        self.inventory.search_by = ApiFields.device.HOSTNAME
        assert self.inventory.search_by == ApiFields.device.HOSTNAME
    ...
    def test_device_is_present_by_hostname(self, CV_DEVICE):
        logging.info("Search device {} in Cloudvision".format(
            CV_DEVICE[ApiFields.device.HOSTNAME]))
        logging.info("Start CV query at {}".format(time_log()))
        requests.packages.urllib3.disable_warnings()
        assert self.inventory.is_device_exist(
            device_lookup=CV_DEVICE[ApiFields.device.HOSTNAME], search_mode=ApiFields.device.HOSTNAME) is True
    ...
    def test_device_is_not_present_by_hostname(self, CV_DEVICE_UNKNOWN):
        ...
        self.inventory.search_by = ApiFields.device.HOSTNAME
        assert self.inventory.is_device_exist(
            device_lookup=CV_DEVICE_UNKNOWN[ApiFields.device.HOSTNAME]) is False
        logging.info("End of CV query at {}".format(time_log()))
        logging.info("Device {} is not present on Cloudvision".format(
            CV_DEVICE_UNKNOWN[ApiFields.device.HOSTNAME]))
    ...
    def test_device_in_container_by_hostname(self, CV_DEVICE):
        ...
        assert self.inventory.is_in_container(
            device_lookup=CV_DEVICE[ApiFields.device.HOSTNAME], container_name=CV_DEVICE[ApiFields.generic.PARENT_CONTAINER_NAME])
        logging.info("End of CV query at {}".format(time_log()))
        logging.info("Device {} is correctly configured under {}".format(
            CV_DEVICE[ApiFields.device.HOSTNAME], CV_DEVICE[ApiFields.generic.PARENT_CONTAINER_NAME]))
    ...
    def test_device_not_in_container_by_hostname(self, CV_DEVICE_UNKNOWN):
        ...
        self.inventory.search_by = ApiFields.device.HOSTNAME
        assert self.inventory.is_in_container(
            device_lookup=CV_DEVICE_UNKNOWN[ApiFields.device.HOSTNAME], container_name=CV_DEVICE_UNKNOWN[ApiFields.device.HOSTNAME]) is False
        logging.info("End of CV query at {}".format(time_log()))
    ...
    def test_get_device_facts_by_hostname(self, CV_DEVICE):
        ...
        device_facts = self.inventory.get_device_facts(
            device_lookup=CV_DEVICE[ApiFields.device.HOSTNAME])
        logging.info("End of CV query at {}".format(time_log()))
        assert device_facts is not None
        assert device_facts[ApiFields.device.FQDN].split(
            ".")[0] == CV_DEVICE[ApiFields.device.HOSTNAME]
        logging.info("Facts for device {} are correct: {}".format(
            CV_DEVICE[ApiFields.device.HOSTNAME], device_facts))
    ...
    def test_get_device_id_by_hostname(self, CV_DEVICE):
        ...
        logging.info("End of CV query at {}".format(time_log()))
        assert device_facts is not None
        assert device_facts == CV_DEVICE[ApiFields.device.SYSMAC]
    ...
    def test_container_id_by_hostname(self, CV_DEVICE):
        ...
        logging.info("End of CV query at {}".format(time_log()))
        assert result[ApiFields.device.ID] == self.cvp.api.get_container_by_name(
            CV_DEVICE[ApiFields.generic.PARENT_CONTAINER_NAME])[ApiFields.device.ID]
    ...
    def test_device_move_by_hostname(self, CV_DEVICE):
        ...
        if resp[0].results["success"]:
            ...
            logging.info("Move device {} to {} with result: {}".format(
                CV_DEVICE[ApiFields.device.HOSTNAME], CV_DEVICE[ApiFields.generic.PARENT_CONTAINER_NAME], resp[0].results))
    ...
    def test_device_manager(self, CV_DEVICE):
        ...
        logging.info("Start CV query at {}".format(time_log()))
        self.inventory.search_by = ApiFields.device.FQDN
        resp = self.inventory.manager(user_inventory=user_inventory, search_mode=ApiFields.device.FQDN)
        logging.info("End of CV query at {}".format(time_log()))
    ...
    def test_container_name(self, CV_DEVICE):
        ...
        for device in user_inventory.devices:
            ...
            result = self.inventory.get_device_container(
                device_lookup=device.fqdn)[ApiFields.generic.PARENT_CONTAINER_NAME]
            cv_result = self.cvp.api.get_device_by_name(
                device.fqdn)[ApiFields.device.CONTAINER_NAME]
    ...
    def test_container_id(self, CV_DEVICE):
        ...
        for device in user_inventory.devices:
            result = self.inventory.get_device_container(
                device_lookup=device.hostname)[ApiFields.generic.PARENT_CONTAINER_ID]
            cv_result = self.cvp.api.get_device_by_name(
                device.fqdn)[ApiFields.generic.PARENT_CONTAINER_ID]
            assert result == cv_result
            logging.info(
                "Collection: {} - CV: {}".format(result, cv_result))
    ...
    def test_get_device_by_sysmac(self, CV_DEVICE):
        ...
        user_inventory = DeviceInventory(data=[CV_DEVICE])
        for device in user_inventory.devices:
            if ApiFields.device.SYSMAC in device.info and device.info[ApiFields.device.SYSMAC] is not None:
                self.inventory.search_by = ApiFields.device.SYSMAC
                device_info = self.inventory.get_device_facts(
                    device_lookup=device.system_mac)
                assert device_info[ApiFields.device.FQDN] == device.fqdn
                assert device_info[ApiFields.device.SYSMAC] == device.system_mac
                self.inventory.search_by = ApiFields.device.FQDN
                ...
                pytest.skip("Skipped as device {} has no {} field".format(
                    device.fqdn, ApiFields.device.SYSMAC))
