...
def cvp_database():
    database = mock.MockCVPDatabase(
        devices=magickmock_data.MOCKDATA_DEVICES,
        configlets=magickmock_data.MOCKDATA_CONFIGLETS,
        containers=magickmock_data.MOCKDATA_CONTAINERS,
        configlets_mappers=magickmock_data.MOCKDATA_CONFIGLET_MAPPERS
    )
    yield database
...
@pytest.mark.parametrize("container_def", generate_list_from_dict(magickmock_data.MOCKDATA_CONTAINERS), ids=generate_test_ids_dict)
...
@pytest.mark.usefixtures("fact_unit_tools")
@pytest.mark.usefixtures("cvp_database")
@pytest.mark.parametrize("configlet_def", magickmock_data.MOCKDATA_CONFIGLET_MAPPERS['data']['configlets'], ids=generate_test_ids_list)
...
@pytest.mark.usefixtures("fact_unit_tools")
@pytest.mark.usefixtures("cvp_database")
@pytest.mark.parametrize("device_def", magickmock_data.MOCKDATA_DEVICES, ids=generate_test_ids_list)
def test_CvFactsTools__device_get_configlets_with_configlets(fact_unit_tools, device_def):
    ...
    LOGGER.info('** facts_tool response: %s', str(result))
    if device_def['systemMacAddress'] in [x['objectId'] for x in magickmock_data.MOCKDATA_CONFIGLET_MAPPERS['data']['configletMappers']]:
        assert len(result) > 0
...
def test_CvFactsTools__device_get_configlets_no_configlet(fact_unit_tools, device_def):
    ...
    result = fact_unit_tools._CvFactsTools__device_get_configlets(netid=device_def['systemMacAddress'])
    LOGGER.info('** facts_tool response: %s', str(result))
    if device_def['systemMacAddress'] not in [x['objectId'] for x in magickmock_data.MOCKDATA_CONFIGLET_MAPPERS['data']['configletMappers']]:
        assert len(result) == 0
...
def test_CvFactsTools__device_get_configlets_with_configlets(fact_unit_tools, container_def):
    ...
    LOGGER.info('** facts_tool response: %s', str(result))
    if container_def[mock.MockCVPDatabase.FIELD_KEY] in [(x['objectId'], x['containerId']) for x in magickmock_data.MOCKDATA_CONFIGLET_MAPPERS['data']['configletMappers']]:
        assert len(result) > 0
...
def test_CvFactsTools__device_get_configlets_no_configlet(fact_unit_tools, device_def):
    ...
    if device_def['parentContainerKey'] != '':
        assert result[mock.MockCVPDatabase.FIELD_PARENT_NAME] in [x['name'] for x in generate_list_from_dict(magickmock_data.MOCKDATA_CONTAINERS)]
