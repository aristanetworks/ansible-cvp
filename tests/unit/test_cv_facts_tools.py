...
@pytest.mark.parametrize("container_def", generate_list_from_dict(mock.MockCVPDatabase.CVP_DATA_CONTAINERS_INIT), ids=generate_test_ids_dict)
...
@pytest.mark.usefixtures("fact_unit_tools")
@pytest.mark.usefixtures("cvp_database")
@pytest.mark.parametrize("device_def", mock.MockCVPDatabase.CVP_DATA_DEVICE_INIT, ids=generate_test_ids_list)
def test_CvFactsTools__device_get_configlets_with_configlets(fact_unit_tools, device_def):
    ...
    LOGGER.info('** facts_tool response: %s', str(result))
    if device_def['systemMacAddress'] in [x['objectId'] for x in mock.MockCVPDatabase.CVP_DATA_CONFIGLETS_MAPPERS_INIT['data']['configletMappers']]:
        assert len(result) > 0
...
def test_CvFactsTools__device_get_configlets_no_configlet(fact_unit_tools, device_def):
    ...
    result = fact_unit_tools._CvFactsTools__device_get_configlets(netid=device_def['systemMacAddress'])
    LOGGER.info('** facts_tool response: %s', str(result))
    if device_def['systemMacAddress'] not in [x['objectId'] for x in mock.MockCVPDatabase.CVP_DATA_CONFIGLETS_MAPPERS_INIT['data']['configletMappers']]:
        assert len(result) == 0
...
def test_CvFactsTools__device_get_configlets_with_configlets(fact_unit_tools, container_def):
    ...
    LOGGER.info('** facts_tool response: %s', str(result))
    if container_def[mock.MockCVPDatabase.FIELD_KEY] in [(x['objectId'], x['containerId']) for x in mock.MockCVPDatabase.CVP_DATA_CONFIGLETS_MAPPERS_INIT['data']['configletMappers']]:
        assert len(result) > 0
...
def test_CvFactsTools__device_get_configlets_no_configlet(fact_unit_tools, device_def):
    ...
    if device_def['parentContainerKey'] != '':
        assert result[mock.MockCVPDatabase.FIELD_PARENT_NAME] in [x['name'] for x in generate_list_from_dict(mock.MockCVPDatabase.CVP_DATA_CONTAINERS_INIT)]
