...
Two methods can be used get Ansible up and running quickly with all the requirements to leverage ansible-cvp.
...
$ docker-compose -f ansible-cvp/development/docker-compose.yml ps
...
- `pylint`: Run python linting with settings defined in [`pylintrc`](https://github.com/aristanetworks/ansible-cvp/blob/devel/pylintrc)
- `yamllint`: Validate all YAML files using configuration from [`yamllintrc`](https://github.com/aristanetworks/ansible-cvp/blob/devel/.github/yamllintrc)
- `ansible-lint`: Validate yaml files are valid against ansible rules.

#### Installation

`pre-commit` is part of [__development requirememnts__](https://github.com/aristanetworks/ansible-cvp/blob/devel/development/requirements-dev.txt). To install, run `pip command` in __ansible-avd__ folder:
...
In a shell, run the following make command. It starts a container in CVP documentation network and leverage [`muffet`](https://github.com/raviqqe/muffet) tool to check 404 HTTP code:

```shell
$ check-cvp-404
docker run --network container:webdoc_cvp raviqqe/muffet \
    http://127.0.0.1:8001 \
    -e ".*fonts.gstatic.com.*" \
    -e ".*edit.*" \
    -f --limit-redirections=3 \
    --timeout=60
http://127.0.0.1:8001/docs/installation/development/
        404     http://127.0.0.1:8001/docs/installation/development/setup-environement2.md
make: *** [check-cvp-404] Error 1
```
