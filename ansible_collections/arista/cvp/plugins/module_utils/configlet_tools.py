...
class CvConfigletTools(object):
    ...
    def apply(self, configlet_list: list, present: bool = True, note: str = 'Managed by Ansible AVD'):
        ...
        for configlet in configlet_list:
            cv_data = self.get_configlet_data_cv(
                configlet_name=configlet[ApiFields.generic.NAME])
            if present:
                if self.is_present(configlet_name=configlet[ApiFields.generic.NAME]):
                    configlet[ApiFields.generic.KEY] = cv_data[ApiFields.generic.KEY]
                    configlet['diff'] = self._compare(
                        fromText=cv_data[ApiFields.generic.CONFIG], toText=configlet[ApiFields.generic.CONFIG], fromName='CVP', toName='Ansible')
            ...
            elif self.is_present(configlet_name=configlet[ApiFields.generic.NAME]):
                configlet[ApiFields.generic.KEY] = cv_data[ApiFields.generic.KEY]
                configlet['diff'] = self._compare(
                    fromText=cv_data[ApiFields.generic.CONFIG], toText=configlet[ApiFields.generic.CONFIG], fromName='CVP', toName='Ansible')
    ...
    def update(self, to_update: list, note: str = 'Managed by Ansible AVD'):
        ...
        for configlet in to_update:
            change_response = CvApiResult(action_name=configlet[ApiFields.generic.NAME])
            if self._ansible.check_mode:
                ...
                MODULE_LOGGER.info('[check mode] - Configlet %s updated on cloudvision', str(
                    configlet[ApiFields.generic.NAME]))
                ...
                try:
                    update_resp = self._cvp_client.api.update_configlet(config=configlet[ApiFields.generic.CONFIG],
                                                                        key=configlet[ApiFields.generic.KEY],
                                                                        name=configlet[ApiFields.generic.NAME],
                                                                        wait_task_ids=True)
                    ...
                    message = "Configlet %s cannot be updated - %s" % (
                        configlet[ApiFields.generic.NAME], errorMessage)
                    ...
                    MODULE_LOGGER.error('Error updating configlet %s: %s', str(
                        configlet[ApiFields.generic.NAME]), str(error))
                    ...
                    if "errorMessage" in str(update_resp):
                        ...
                        # Build error message to report in ansible output
                        message = "Configlet %s cannot be updated - %s" % (
                            configlet[ApiFields.generic.NAME], update_resp['errorMessage'])
                        # Add logging to ansible response.
                        change_response.add_entry(message)
                        # Generate logging error message
                        MODULE_LOGGER.error('Error updating configlet %s: %s', str(
                            configlet[ApiFields.generic.NAME]), str(update_resp['errorMessage']))
                        self._ansible.fail_json(msg=message)
                    else:
                        ...
                        self._cvp_client.api.add_note_to_configlet(
                            configlet[ApiFields.generic.KEY], configlets_notes)
                        # Save configlet diff
                        ...
                        if 'diff' in configlet:
                            ...
                            if configlet['diff'] is not None and configlet['diff'][0] is True:
                                ...
                                MODULE_LOGGER.info(
                                    'Found diff in configlet %s.', str(configlet[ApiFields.generic.NAME]))
                        if 'notediff' in configlet:
                            if configlet['notediff'] is not None and configlet['notediff'][0] is True:
                                ...
                                change_response.changed = True
                                MODULE_LOGGER.info(
                                    'Found diff in configlet note of configlet %s.', str(configlet[ApiFields.generic.NAME]))
                        ...
                        MODULE_LOGGER.info(
                            'Configlet %s updated on cloudvision', str(configlet[ApiFields.generic.NAME]))
    ...
    def create(self, to_create, note: str = 'Managed by Ansible AVD'):
        ...
        for configlet in to_create:
            ...
            if self._ansible.check_mode:
                ...
                try:
                    new_resp = self._cvp_client.api.add_configlet(name=configlet[ApiFields.generic.NAME], config=configlet[ApiFields.generic.CONFIG])
                except Exception as error:
                    # Mark module execution with error
                    # Build error message to report in ansible output
                    errorMessage = re.split(':', str(error))[-1]
                    ...
                    if "errorMessage" in str(new_resp):
                        ...
                        # Build error message to report in ansible output
                        message = "Configlet %s cannot be created - %s" % (
                            configlet[ApiFields.generic.NAME], new_resp['errorMessage'])
                        # Add logging to ansible response.
                        change_response.add_entry(message)
                        # Generate logging error message
                        MODULE_LOGGER.error(
                            'Error creating configlet %s: %s', str(configlet[ApiFields.generic.NAME]), str(new_resp))
                        self._ansible.fail_json(msg=message)
                    else:
                        ...
                        change_response.changed = True
                        change_response.success = True
                        MODULE_LOGGER.info('Configlet %s created on cloudvision', str(configlet[ApiFields.generic.NAME]))
            response_data.append(change_response)
        return response_data
            ...
            if self._ansible.check_mode:
                ...
                try:
                    delete_resp = self._cvp_client.api.delete_configlet(
                        name=configlet[ApiFields.generic.NAME], key=configlet[ApiFields.generic.KEY])
                except Exception as error:
                    # Mark module execution with error
                    # Build error message to report in ansible output
                    errorMessage = re.split(':', str(error))[-1]
                    ...
                    if "errorMessage" in str(delete_resp):
                        ...
                        # Build error message to report in ansible output
                        message = "Configlet %s cannot be deleted - %s" % (
                            configlet[ApiFields.generic.NAME], delete_resp['errorMessage'])
                        # Add logging to ansible response.
                        change_response.add_entry(message)
                        # Generate logging error message
                        MODULE_LOGGER.error(
                            'Error deleting configlet %s: %s', str(configlet[ApiFields.generic.NAME]), str(delete_resp))
                        self._ansible.fail_json(msg=message)
                    else:
                        change_response.add_entry('configlet deleted')
                        change_response.changed = True
                        change_response.success = True  # noqa # pylint: disable=unused-variable
                        MODULE_LOGGER.info('Configlet %s deleted on cloudvision', str(configlet[ApiFields.generic.NAME]))
            response_data.append(change_response)
        return response_data
